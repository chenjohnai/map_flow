...
#            print line[i].split("\t")
            file_traj_add.writelines(str(line[i] + "\n"))
            if i % 1000 == 0:
                print i
            ...
            node_change = False
            search4nodes(start_node, end_node, node_change, file_traj_add, file_traj_add, csv_file, another_start_node)


def search4nodes(start_node, end_node, node_change, file_traj_add, csv_file, another_start_node):
        ...
        try:
            ...
            file_traj_add.writelines("\n")
            return "there are node end node ways"
            continue
        ...
        way_query = "SELECT ways_index.ways_index FROM public.ways_index WHERE id = %d"                
        node_cursor.execute(way_query% long(start_node_way[0]))
        start_linkingways = node_cursor.fetchall()[0][0]
        start_end_set = set(start_linkingways) & set(end_ways)
        if start_end_set:
            search_node_query = "SELECT nodes FROM public.planet_osm_ways WHERE id = %d"
            
            osm_cursor.execute(search_node_query% long(start_node_way[0]))
            start_nodelist = osm_cursor.fetchall()[0][0]
            osm_cursor.execute(search_node_query% long(start_end_set[0]))
            end_node_list = osm_cursor.fetchall()[0][0]
            
            internode = long(set(start_nodelist) & set(end_node_list))
            
            start_pos = start_nodelist.index(long(start_node))
            inter_pos_st = start_nodelist.index(internode[0])
            if inter_pos_st >= start_pos:
                single_traj = start_nodelist[start_pos: inter_pos_st + 1]
            else:
                trajtmp = start_nodelist[inter_pos_st: start_pos + 1]
                trajtmp.inverse()
                single_traj = trajtmp
            
            end_pos = end_node_list.index(long(end_node))
            inter_pos_end = end_node_list.index(internode[0])
            if end_pos >= inter_pos_end:
                trajtmp = end_node_list[inter_pos_end: end_pos + 1]
                single_traj.extend(trajtmp)
            else:
                trajtmp = end_node_list[end_pos : inter_pos_end + 1]
                trajtmp.inverse()
                single_traj.extend(trajtmp)
